...
describe('Keyboard Manager', () => {
  ...
  describe('with menuitemcheckbox components', () => {
    ...
    const grabElementsForTesting = () => {
      ...
      if (nativeMenus.length > 0) {
        fontMenuItems = fixture.componentInstance.menuItems.toArray().slice(0, 2);
      }
    };
    ...
    <div>
      ...
      <ng-template cdkMenuPanel #share="cdkMenuPanel">
        <div cdkMenu id="share_menu" [cdkMenuPanel]="share">
          <button (cdkMenuItemOnTrigger)="clickEmitter.next()" cdkMenuItem>Email</button>
...
class MenuWithCheckboxes {
  ...
  @ViewChildren(CdkMenuItemCheckbox) menuItems: QueryList<CdkMenuItemCheckbox>;
}
...
class MenuWithRadioButtons {
  ...
  @ViewChildren(CdkMenuItemRadio) menuItems: QueryList<CdkMenuItemRadio>;
}
