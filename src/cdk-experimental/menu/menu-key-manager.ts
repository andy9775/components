...
export class MenuKeyManager {
  ...
  constructor(
    children: QueryList<CdkMenuItem>,
    private readonly _dir: Directionality,
    private _menuOrientation: 'horizontal' | 'vertical',
    debounce = defaultTypeAheadDebounce
  ) {
    this._keyManager = new FocusKeyManager(children).withWrap().withTypeAhead(debounce);

    if (this._isHorizontal()) {
      this._keyManager.withHorizontalOrientation(_dir.value);
    } else {
   ...
   * @param menuItem the triggering menu item who's attached menu's Keyboard Manager it subscribes
   * to.
   */
  private _subscribeToBubbledEvents(menuItem: CdkMenuItem) {
    ...
    if (menuItem.hasMenu()) {
      ...
      menuItem
        ...
        .subscribe(() => {
          const menu = menuItem.getRenderedMenu();
          if (menu && menu._getBubbledKeyboardEvents()) {
            menu
  ...
  onKeydown(event: KeyboardEvent, bubbled = false) {
    // Since a menu item opens its menu on right arrow when in ltr layout and vertical
    // we need to open on left arrow if the layout is rtl.
    const _RIGHT_ARROW =
      this._dir.value === 'ltr' || this._isHorizontal() ? RIGHT_ARROW : LEFT_ARROW;
    const _LEFT_ARROW =
      this._dir.value === 'ltr' || this._isHorizontal() ? LEFT_ARROW : RIGHT_ARROW;
    ...
    switch (event.keyCode) {
      ...
      case _RIGHT_ARROW:
        if (this._isHorizontal()) {
          this._focusNext(event);
        } else if (activeItem && activeItem.hasMenu() && !bubbled) {
          this._openActiveMenu();
          this._focusFirstSubmenuItem();
        } else {
          this._closeActiveMenu();
      ...
      case UP_ARROW:
      case DOWN_ARROW:
        if (this._isHorizontal() && activeItem && activeItem.hasMenu()) {
  ...
  private _openActiveMenu() {
    const activeItem = this._keyManager.activeItem;
    if (activeItem && activeItem.hasMenu() && !activeItem.isMenuOpen()) {
      activeItem.trigger();
    }
  }
  ...
  private _focusFirstSubmenuItem() {
    const activeItem = this._keyManager.activeItem;
    if (activeItem && activeItem.isMenuOpen()) {
      activeItem.getRenderedMenu()!.focusFirstItem();
    }
  }
  ...
  private _focusLastSubmenuItem() {
    const activeItem = this._keyManager.activeItem;
    if (activeItem && activeItem.isMenuOpen()) {
      activeItem.getRenderedMenu()!.focusLastItem();
    }
  }
  ...
  private _isActiveMenuOpen() {
    const activeItem = this._keyManager.activeItem;
    return activeItem && activeItem.hasMenu() && activeItem.isMenuOpen();
  }
  ...
  private _focusNext(event: KeyboardEvent) {
    ...
    if (previous && previous !== current && previous.isMenuOpen()) {
  ...
  private _isHorizontal() {
    return this._menuOrientation === 'horizontal';
  }
