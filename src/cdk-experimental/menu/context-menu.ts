...
// TODO refactor this out as the general logic is shared between this and the MenuBar
/**
...
export class ContextMenuTracker {
  ...
  update(trigger: CdkContextMenuTrigger) {
    if (ContextMenuTracker._openContextMenuTrigger !== trigger) {
      ContextMenuTracker._openContextMenuTrigger?.closeMenu();
...
export class CdkContextMenuTrigger implements OnDestroy {
  ...
  @Input('cdkContextMenuDisabled') disabled = false;
  ...
  _openOnContextMenu(event: MouseEvent) {
    if (!this.disabled) {
      ...
      // We stop event propagation to ensure that only the lowest level enabled context menu gets
      // displayed. Otherwise any parent context menu(s) in which this one sits would also fire-off
  ...
  private _setCloseListener() {
    merge(fromEvent(this._document, 'click'), fromEvent(this._document, 'contextmenu'))
      .pipe(take(1), takeUntil(this._stopDocumentListener))
      .subscribe(event => {
