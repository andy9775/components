...
describe('Menu', () => {
  ...
  describe('menu aim with ltr layout and at top of page moving down and right', () => {
    ...
    function hover(from: Point, to: Point, inMenu: HTMLElement, duration: number) {
      const getNextPoint = getNextPointClosure(from, to);
     ...
     * another.
     */
    function getNextPointClosure(from: Point, to: Point) {
      let x = from.x;
  ...
  describe('menu aim with rtl layout and at bottom of page moving up and left', () => {
...
@Component({
  template: `
    <div style="height: 100vh; bottom: 0;">
      <div cdkMenuBar style="position: absolute; bottom: 0;">
        <button #file_trigger cdkMenuItem [cdkMenuTriggerFor]="file">File</button>
      </div>
